...
from .utils import is_valid_ipv4, non_blocking_read
...
class TUNInterface:
    ...
    def __init__(self):
        ...
        self.tun_name: str = config.tun_name
        self.running = False
    ...
    def _read_loop(self):
        ...
        while self.running:
            try:
                # Read packet
                packet = non_blocking_read(self.tun_fd, config.max_packet_size)
                if packet and is_valid_ipv4(packet):
                    batch.append(packet)
                
                # Process batch
                if batch and (len(batch) >= config.batch_size or not packet):
